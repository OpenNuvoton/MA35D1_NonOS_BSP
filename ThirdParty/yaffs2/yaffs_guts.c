...
u8 *yaffs_get_temp_buffer(struct yaffs_dev * dev)
{
	...
	return yaffs_malloc(dev->data_bytes_per_chunk);
...
YCHAR *yaffs_clone_str(const YCHAR *str)
{
	...
	new_str = yaffs_malloc((len + 1) * sizeof(YCHAR));
	if (new_str) {
				   ...
				   enum yaffs_obj_type type)
{
	...
	case YAFFS_OBJECT_TYPE_SYMLINK:
		...
		yaffs_free(obj->variant.symlink_variant.alias);
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
	...
	/* Finished verifying the device, continue with initialization */
...
void yaffs_deinitialise(struct yaffs_dev *dev)
{
	if (dev->is_mounted) {
		...
		yaffs_free(dev->checkpt_buffer);
		dev->checkpt_buffer = NULL;
		//kfree(dev->checkpt_block_list);
		yaffs_free(dev->checkpt_block_list);
