...
void nand_wait_ready(struct mtd_info *mtd)
{
	...
	while ((msTicks0 - time_start) < timeo) {
		if (chip->dev_ready)
...
static int nand_wait(struct mtd_info *mtd, struct nand_chip *chip)
{
	...
	if (ret)
 	...
 	time_start = msTicks0;
 	while ((msTicks0 - time_start) < timer) {
		if (chip->dev_ready) {
			if (chip->dev_ready(mtd))
				break;
		} else {
...
static int nand_init_data_interface(struct nand_chip *chip)
{
	...
	chip->data_interface = yaffs_malloc(sizeof(*chip->data_interface));
...
int nand_scan_tail(struct mtd_info *mtd)
{
	...
	if (!(chip->options & NAND_OWN_BUFFERS)) {
		...
		//chip->buffers = nbuf;
		ptr = yaffs_malloc(sizeof(struct nand_buffers) + 31);
        chip->buffers = (struct nand_buffers *) ((uint64_t)((char *)ptr+31) & (~0x1f));
