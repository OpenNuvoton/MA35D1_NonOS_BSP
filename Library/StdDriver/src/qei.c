...
void QEI_DisableInt(QEI_T* qei, uint32_t u32IntSel)
{
    ...
    if(qei ==(QEI_T*)QEI0)
    {
    	IRQ_Disable((IRQn_Type)QEI0_IRQn);
    }
    else if(qei ==(QEI_T*)QEI1)
    {
    	IRQ_Disable((IRQn_Type)QEI1_IRQn);
    }else
    {
        IRQ_Disable((IRQn_Type)QEI2_IRQn);
    }
}
...
void QEI_EnableInt(QEI_T* qei, uint32_t u32IntSel)
{
    ...
    if(qei == (QEI_T*)QEI0)
    {
    	IRQ_Enable(QEI0_IRQn);
    }
    else if(qei == (QEI_T*)QEI1)
    {
    	IRQ_Enable(QEI1_IRQn);
    }else
    {
        IRQ_Enable(QEI2_IRQn);
    }
}
