/**************************************************************************//**
 * @file     cdc_core.c
 * @brief    USB Host CDC driver core
 *
...
/// @cond HIDDEN_SYMBOLS
#define USB_XFER_TIMEOUT             1000    /* 1000 ms */
...
int32_t  usbh_cdc_get_line_coding(CDC_DEV_T *cdev, LINE_CODING_T *line_code)
{
    ...
    memcpy(line_code, cdev->cmd_buff, 7);
    return ret;
}
...
int32_t  usbh_cdc_set_line_coding(CDC_DEV_T *cdev, LINE_CODING_T *line_code)
{
	...
	memcpy(cdev->cmd_buff, line_code, 7);
 ...
 *                       Note that this buffer must be non-cache. 
