...
{
	return usbh_alloc_mem(sizeof(UAC_DEV_T));
}

static void  free_uac_device(UAC_DEV_T *uac)
{
	usbh_free_mem(uac, sizeof(UAC_DEV_T));
}
...
static void  uac_disconnect(IFACE_T *iface)
{
	...
	p = find_uac_device(iface->udev);
	if (p == NULL) {
		UAC_ERRMSG("%s - Cannot find UAC device!\n", __func__);
		return;
	}

	if (uac != p) {
		UAC_ERRMSG("%s - Serious error! UAC device not matched!\n", __func__);
		return;
	}
	...
	if (uac->asif_in.iface == iface) {
		usbh_uac_stop_audio_in(uac);
		uac->asif_in.iface = NULL;
	}

	if (uac->asif_out.iface == iface) {
	...
	if (uac->acif.iface || uac->asif_in.iface || uac->asif_out.iface)
		return;   /* some other interface still not closed */
