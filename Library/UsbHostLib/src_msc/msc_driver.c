...
void msc_reset(MSC_T *msc)
{
    ...
    ret = usbh_ctrl_xfer(udev, REQ_TYPE_OUT | REQ_TYPE_CLASS_DEV | REQ_TYPE_TO_IFACE,
                         0xFF, 0, msc->iface->if_num, 0, NULL, &read_len, 1000);
    if (ret < 0)
    {
...
static int  msc_inquiry(MSC_T *msc)
{
    ...
    ret = run_scsi_command(msc, msc->scsi_buff, 36, 1, 1000);
    if (ret < 0)
    {
...
static int  msc_request_sense(MSC_T *msc)
{
    ...
    ret = run_scsi_command(msc, msc->scsi_buff, 18, 1, 1000);
    if (ret < 0)
    {
...
static int  msc_test_unit_ready(MSC_T *msc)
{
    ...
    ret = run_scsi_command(msc, msc->scsi_buff, 0, 1, 1000);
    if (ret < 0)
    {
        if (ret == USBH_ERR_STALL)
            msc_reset(msc);
...
/**
  ...
  *                        It must be non-cache.
  * @return
  *              - 0    Success
  *              - \ref UMAS_ERR_DRIVE_NOT_FOUND   There's no mass storage device mounted to this volume.
  *              - \ref UMAS_ERR_IO      Failed to read disk.
  */
int  usbh_umas_read(int drv_no, uint32_t sec_no, int sec_cnt, uint8_t *buff)
{
    ...
    ret = run_scsi_command(msc, buff, sec_cnt * 512, 1, 2000);
    if (ret != 0)
    {
...
/**
  ...
  *              - 0       Success
  *              - \ref UMAS_ERR_DRIVE_NOT_FOUND   There's no mass storage device mounted to this volume.
  *              - \ref UMAS_ERR_IO      Failed to write disk.
  */
int  usbh_umas_write(int drv_no, uint32_t sec_no, int sec_cnt, uint8_t *buff)
{
    ...
    ret = run_scsi_command(msc, buff, sec_cnt * 512, 0, 2000);
    if (ret < 0)
    {
...
/**
  ...
  *
  * @return
  *              - \ref UMAS_OK   Mass storage device is ready.
  *              - \ref UMAS_ERR_DRIVE_NOT_FOUND   There's no mass storage device mounted to this volume.
  *              - \ref UMAS_ERR_IVALID_PARM       Failed to write disk.
  */
...
disk_found:
        ...
        for (retries = 0; retries < 3; retries++)
        {
            ...
            ret = run_scsi_command(try_msc, try_msc->scsi_buff, 8, 1, 1000);
            if (ret < 0)
            {
