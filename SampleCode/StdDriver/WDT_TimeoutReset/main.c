 ...
 * @file     main.c
 *
 * @brief    Implement WDT time-out interrupt and generate time-out reset
 *           system event while WDT time-out reset delay period expired.
 *
...
int main(void)
{
    ...
    sysprintf("+-------------------------------------------------+\n");
    sysprintf("|            WDT Time-out Sample Code             |\n");
    sysprintf("+-------------------------------------------------+\n");
    sysprintf("| WDT Settings:                                   |\n");
    sysprintf("|    - Clock source is LIRC @ 32 kHz              |\n");
    sysprintf("|    - Time-out interval is 2^14 * WDT clock      |\n");
    sysprintf("|      (around 0.512 s)                           |\n");
    sysprintf("|    - Interrupt enable                           |\n");
    sysprintf("|    - Reset function enable                      |\n");
    sysprintf("|    - Reset delay period is 18 * WDT clock       |\n");
    sysprintf("|                                                 |\n");
    sysprintf("| System will generate a WDT time-out interrupt   |\n");
    sysprintf("| event after 0.512 s.                            |\n");
    sysprintf("| Use PA.0 high/low period time to check WDT      |\n");
    sysprintf("| time-out interval.                              |\n");
    sysprintf("|                                                 |\n");
    sysprintf("| When WDT interrupt counts large than 10, we     |\n");
    sysprintf("| will not reset WDT counter value and system     |\n");
    sysprintf("| will be reset immediately by WDT time-out reset |\n");
    sysprintf("| signal.                                         |\n");
    sysprintf("+-------------------------------------------------+\n\n");
    ...
    if (u8Item == 0)
    {
        ...
        WDT_Open(WDT0, WDT_TIMEOUT_2POW14, WDT_RESET_DELAY_18CLK, TRUE, FALSE);
    ...
    else
    {
        ...
        WDT_Open(WDT1, WDT_TIMEOUT_2POW14, WDT_RESET_DELAY_18CLK, TRUE, FALSE);
