/*************************************************************************//**
 * @file     net.c
 * @brief    Packet processor source file for MA35D1
 *
...
static void arp_reply(int intf, uint8_t *target_ip, uint8_t *target_mac)
{
    ...
    if(intf == GMACINTF0)
        GMAC0_TransmitPkt(NULL, &au8TxBuf[0], sizeof(ARP_PACKET));
    else
        GMAC1_TransmitPkt(NULL, &au8TxBuf[0], sizeof(ARP_PACKET));
}
                      ...
                      uint16_t u16DestPort, uint8_t *pu8TxData, uint32_t u32Len)
{
    ...
    if(intf == GMACINTF0)
        GMAC0_TransmitPkt(NULL, &au8TxBuf[0], sizeof(UDP_PACKET) + u32Len);
    else
        GMAC1_TransmitPkt(NULL, &au8TxBuf[0], sizeof(UDP_PACKET) + u32Len);
    // To enable RX IRQ ?
}
    ...
    else                        /* this is a multicast or unicast packet */
    {
                ...
                (pu8Packet[34] == 0x08))
        {
            ...
            *(uint16_t *)&au8TxBuf[36] = ~chksum((uint16_t *)&au8TxBuf[34], (u32Len - 34) / 2);
            ...
            if(intf == GMACINTF0)
                GMAC0_TransmitPkt(NULL, au8TxBuf, u32Len);
            else
                GMAC1_TransmitPkt(NULL, au8TxBuf, u32Len);
...
void GMAC0_IRQHandler(void)
{
    ...
    {
        process_rx_packet(GMACINTF0, (uint8_t *)((u64)(rskb->data)), rskb->len);
    }
}
...
void GMAC1_IRQHandler(void)
{
    ...
    {
        process_rx_packet(GMACINTF1, (uint8_t *)((u64)(rskb->data)), rskb->len);
    }
}
...
int32_t GMAC0_TransmitPkt(struct sk_buff *ptskb, uint8_t *pbuf, uint32_t len)
{
    ...
    if(ptskb == NULL)
    {
        ...
        return GMAC_xmit_frames(tskb, GMACINTF0, 1, 0);
    }
    else
    {
        ptskb->len = len;
        return GMAC_xmit_frames(ptskb, GMACINTF0, 1, 0);
    }
}
...
int32_t GMAC1_TransmitPkt(struct sk_buff *ptskb, uint8_t *pbuf, uint32_t len)
{
    ...
    if(ptskb == NULL)
    {
        ...
        memcpy((uint8_t *)((u64)(tskb->data)), pbuf, len);
        dcache_clean_invalidate_by_mva(tskb->data, (size_t)tskb->len);
        return GMAC_xmit_frames(tskb, GMACINTF1, 1, 0);
    }
    else
    {
        ptskb->len = len;
        return GMAC_xmit_frames(ptskb, GMACINTF1, 1, 0);
    }
}
