...
int32_t NOR_MX29LV320T_EraseChip(uint32_t u32Bank, uint32_t u32IsCheckBlank)
{
    ...
    if(u32IsCheckBlank)
    {
        ...
        for(u32Addr = 0; u32Addr < EBI_MAX_SIZE; u32Addr += 2)
        {
            ...
            if(u16RData != 0xFFFF)
            {
                sysprintf("> Chip erase done and blank check FAIL on 0x%08X (R:0x%X)!!!\n\n", u32Addr, u16RData);
                return -1;
            }
        }
        sysprintf(">> Chip erase done and blank check OK !!!\n\n");
    }
    else
    {
        sysprintf(">> Chip erase done !!!\n\n");
    }
