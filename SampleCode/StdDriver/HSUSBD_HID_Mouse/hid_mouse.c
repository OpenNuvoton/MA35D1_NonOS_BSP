...
void HID_Init(void)
{
	...
	HSUSBD_ENABLE_EP_INT(HSUSBD_DAINTMSK_InEpMsk0_Msk | HSUSBD_DAINTMSK_OutEPMsk0_Msk | HSUSBD_DAINTMSK_InEpMsk1_Msk);
    ...
    if (g_u8EP1Ready)
    {
        ...
        if (mouse_mode)
        {
            if (move_len > 8)
            {
            	...
            	ep1_buf_in[0] = 0x00;
            	ep1_buf_in[1] = mouse_table[mouse_idx & 0x07];
            	ep1_buf_in[2] = mouse_table[(mouse_idx + 2) & 0x07];
            	ep1_buf_in[3] = 0x00;
                mouse_idx++;
                move_len = 0;
            }
