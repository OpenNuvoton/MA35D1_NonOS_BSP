        ...
        while(u32Idx < u32EBIsize)
        {
            if(u32WriteMode == 0)           // From Byte Write Mode
            {
                ...
                if(EBI0_READ_DATA8(u32Idx) != (uint8_t)(u32CmpData))
                {
                    ...
                    sysprintf("SRAM want data= 0x%x\n", (uint8_t)(u32CmpData));
                    sysprintf("SRAM read data= 0x%x\n",            EBI0_READ_DATA8(u32Idx));
                    sysprintf("One Byte Read compare failed ! \n\n");
                    while(1);
                }
                u32Idx++;
                u32ReadData++;
        ...
        while(u32Idx < u32EBIsize)
        {
            if(u32WriteMode == 0)           // From Byte Write Mode
            {
                ...
                if(EBI0_READ_DATA16(u32Idx) != (uint16_t)(u32CmpData))
                {
                    ...
                    sysprintf("Half Word Read compare failed ! \n\n");
                    while(1);
                }
                u32Idx += 2;
                u32ReadData += 2;
        ...
        while(u32Idx < u32EBIsize)
        {
            if(u32WriteMode == 0)           // From Byte Write Mode
            {
                ...
                if(EBI0_READ_DATA32(u32Idx) != (uint32_t)(u32CmpData))
                {
                    ...
                    sysprintf("One Word Read compare failed ! \n\n");
                    while(1);
                }
                u32Idx += 4;
                u32ReadData += 4;
...
void SRAM_IS61WV204816BLL(uint32_t u32MaxSize)
{
    ...
    sysprintf(">> Data width is 16-bit and max. data size is %d KB << \n\n", (u32EBIsize / 1024));

    /* One Byte Write */
    sysprintf("[[ One Byte Write test ]]\n");
    u32DataIdx = 0;
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            sysprintf("	All 0x%02X access ... ", (uint8_t)u32WriteData);
            while(u32Idx < u32EBIsize)
            {
        ...
        else
        {
            ...
            u32Idx = 0;
            u32WriteData = 0;
            sysprintf("	0x0, 0x1, 0x2, 0x3, 0x4 ... access with offset(0x%X) ... ", ptr_to_u32(&u32Idx));
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA8(u32Idx, (uint8_t)(u32WriteData + ptr_to_u32(&u32Idx)));
                u32Idx++;
                u32WriteData++;
            }
            ...
            /* Read SRAM data and compare with write in data                                                           */
            /*---------------------------------------------------------------------------------------------------------*/
            SRAM_CompareFunction(u32DataIdx, u32EBIsize, ptr_to_u32(&u32Idx), 0); // param3:0, Byte Write
            sysprintf("PASS\n\n");
        }
        u32DataIdx++;
    }
    ...
    /* Half Word Write */
    sysprintf("[[ Half Word Write test]]\n");
    u32DataIdx = 0;
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            u32Idx = 0;
            u32WriteData = g_au32DataArrary[u32DataIdx];
            sysprintf("	All 0x%04X access ... ", (uint16_t)u32WriteData);
            while(u32Idx < u32EBIsize)
            {
        ...
        else
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA16(u32Idx, (uint16_t)(u32WriteData + ptr_to_u32(&u32Idx)));
                u32Idx += 2;
                u32WriteData++;
            }
            ...
            /* Read SRAM data and compare with write in data                                                           */
            /*---------------------------------------------------------------------------------------------------------*/
            SRAM_CompareFunction(u32DataIdx, u32EBIsize, ptr_to_u32(&u32Idx), 1); // param3:1, Half Word Write
            sysprintf("PASS\n\n");
        }
        u32DataIdx++;
    }
    ...
    while(u32DataIdx < 5)
    {
        if(u32DataIdx < 4)
        {
            ...
            u32Idx = 0;
            u32WriteData = g_au32DataArrary[u32DataIdx];
            sysprintf("	All 0x%08X access ... ", (uint32_t)u32WriteData);
            while(u32Idx < u32EBIsize)
            {
        ...
        else
        {
            ...
            while(u32Idx < u32EBIsize)
            {
                EBI0_WRITE_DATA32(u32Idx, (uint32_t)(u32WriteData + ptr_to_u32(&u32Idx)));
                u32Idx += 4;
                u32WriteData++;
            }
            ...
            /* Read SRAM data and compare with write in data                                                           */
            /*---------------------------------------------------------------------------------------------------------*/
            SRAM_CompareFunction(u32DataIdx, u32EBIsize, ptr_to_u32(&u32Idx), 2); // param3:2, Word Write
            sysprintf("PASS\n\n");
        }
        u32DataIdx++;
    }
