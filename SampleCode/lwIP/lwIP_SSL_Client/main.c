 ...
 * @file     main.c
 *
 * @brief    A TCP echo server which is implemented with LwIP under FreeRTOS.
 *           The server listen to port 80, IP address could configure statically
 *           to 192.168.1.3 or assign by DHCP server. This server replies
...
#define SSL_TASK_PRIORITY        ( tskIDLE_PRIORITY + 3UL )
#define SSL_THREAD_STACKSIZE     ( 2048 )
...
int main(void)
{
    ...
    xTaskCreate( vSslTask, "SslTask", SSL_THREAD_STACKSIZE, NULL, SSL_TASK_PRIORITY, NULL );
