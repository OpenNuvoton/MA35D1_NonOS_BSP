                                    ...
                                    _E_MQTT_TCP_ERRORS * mqtt_tcp_err)
{
    ...
    //sysprintf("mqtt_tcp_connect 00heap:%d\n",xPortGetFreeHeapSize());
    if(info)
    {
        if(conf == 0)
        {
            sysprintf("mqtt_tcp_connect ssl_flag==0,conf==0\n");
            info->ssl_flag = 0;
        ...
        else
        {
            *mqtt_tcp_err = MQTT_TCP_ERROR_NONE;
            ...
            //sysprintf("mqtt_tcp_connect 01heap:%d\n",xPortGetFreeHeapSize());
            info->ssl = lwip_tls_new(conf, NETCONN_TCP);

            //sysprintf("mqtt_tcp_connect 02heap:%d\n",xPortGetFreeHeapSize());
            if(info->ssl)
            {
...
_E_MQTT_TCP_ERRORS mqtt_tcp_recv(_S_MQTT_TCP_INFO * info, struct netbuf ** recv, uint32_t timeout_ms)
{
    ...
    if(info->ssl_flag == 0)
    {
        sysprintf("mqtt_tcp_recv ssl_flag==0\n");
