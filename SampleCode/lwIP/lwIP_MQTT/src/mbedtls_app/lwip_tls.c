...
static uint64_t _lwip_write(void * lwip_info, const uint8_t * buf, uint32_t len);
static uint64_t _lwip_recv(void * lwip_info, uint8_t * buf, uint32_t len);
	...
	if(type == NETCONN_TCP)
	{
		...
		if(ssl)
		{
			...
			if(l)
			{
				...
				if(l->conn)
				{
					sysprintf("new conn=%x\n",l->conn);
					ret = 1;
					ret = mbedtls_ssl_setup(ssl, (mbedtls_ssl_config *)conf);
					mbedtls_ssl_set_bio(ssl, l, (void *)_lwip_write, (void *)_lwip_recv, (void *)_lwip_recv_timeout);
					if(ret < 0)
					{
...
int32_t lwip_tls_read(tls_context_t * ssl, struct netbuf ** _buf, uint32_t timeout_ms)
{
	...
	*_buf = 0;
	...
	if(data)
	{
        ...
        {
			ret = mbedtls_ssl_read((mbedtls_ssl_context *)ssl, (unsigned char*)((uint64_t)data + xRead), (size_t)(max_data_len - xRead));
			if(ret > 0)
			{
...
static uint64_t _lwip_write(void * _lwip_info, const uint8_t * buf, uint32_t len)
{
	uint64_t ret = -1;
...
static uint64_t _lwip_recv(void * _lwip_info, uint8_t * recv_ptr, uint32_t recv_len)
{
	_S_SSL_LWIP_INFO * info = _lwip_info;
