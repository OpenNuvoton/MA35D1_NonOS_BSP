 ...
 *           perform ECC public key generation with the private key from Key Store.
 *
...
static char       pub_kbuff[580 * 2];
static uint32_t   key_buff_pool[20];
static uint32_t   *key_buff;

static int  key_size_tbl[] = { 128, 163, 192, 224, 233, 255, 256, 283, 384, 409, 512, 521, 571, 0, 0, 0,
							   1024, 1536, 2048, 3072, 4096 };

static int  key_size_to_ecc_meta(int ecc_blen)
{
	int   i;

	for (i = 0; i < sizeof(key_size_tbl)/sizeof(int); i++)
	{
		if (key_size_tbl[i] == ecc_blen)
			return i;
	}
	sysprintf("ERROR!! key_size_to_ecc_meta failed! %d\n", ecc_blen);
	while (1);
}
...
int32_t main(void)
{
	...
	pub_x = nc_ptr(pub_kbuff);
	pub_y = nc_ptr(&pub_kbuff[576]);
	key_buff = nc_ptr(key_buff_pool);

	sysprintf("\n\n");
	sysprintf("+---------------------------------------------------------------------------+\n");
	sysprintf("|  MA35D1 TSI ECC generate private key with the public key from Key Store   |\n");
	sysprintf("+---------------------------------------------------------------------------+\n");
	...
	for (i = 0; i < sizeof(_kp)/sizeof(KP_TV_T); i++)
	{
		sysprintf("Run curve %s test.......", _kp[i].curve_name);

		keysz = key_size_to_ecc_meta(_kp[i].keylen);

		memset(key_buff, 0, sizeof(key_buff_pool));
		Hex2Reg((char *)_kp[i].d, key_buff);

		sysprintf("Write ECC private key to KS_SRAM...\n");
		ret = TSI_KS_Write_SRAM(KS_META_ECC | (keysz << KS_METADATA_SIZE_Pos), key_buff, &knum_d);
		if (ret != 0)
			goto tsi_err;

		if (knum_d < 0)
		{
			sysprintf("KS_Write d failed!\n");
			return -1;
		}
		sysprintf("Using KS_SRAM key %d...\n", knum_d);

		// ret = TSI_ECC_GenPublicKey(_kp[i].curve, 0, ECC_KEY_SEL_USER, 0, ptr_to_u32(_kp[i].d), ptr_to_u32(pub_x));
		ret = TSI_ECC_GenPublicKey(_kp[i].curve, 0, ECC_KEY_SEL_KS_SRAM, knum_d, 0, ptr_to_u32(pub_x));
		if (ret != 0)
		{
			sysprintf("TSI_ECC_GenPublicKey failed!\n");
			goto tsi_err;
		}

		if (ecc_strcmp((char *)_kp[i].Qx, pub_x) != 0)
		{
			sysprintf("Public key X [%s] is not matched with expected [%s]!\n", pub_x, _kp[i].Qx);
			while (1);
		}

		if (ecc_strcmp((char *)_kp[i].Qy, pub_y) != 0)
		{
	...
	sysprintf("\n\nECC public key generation test vector compared passed.\n");
	while (1);
