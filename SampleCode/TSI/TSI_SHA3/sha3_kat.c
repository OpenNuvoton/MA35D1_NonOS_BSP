...
#define MAX_SHA_MSG_LEN     (0x10000)

extern uint32_t  VectorDataBase, VectorDataLimit;
extern uint32_t  VectorDataBase_Shake, VectorDataLimit_Shake;
...
static uint8_t  sha3_msg_mem[MAX_SHA_MSG_LEN] __attribute__((aligned(32)));
static uint8_t  tsi_dgst_mem[256] __attribute__((aligned(32)));
static uint8_t  g_sha_digest[1024] __attribute__((aligned(4)));
static uint8_t  *g_sha3_msg;
static int      g_msg_len, g_digest_len;
...
static int  get_line(void)
{
	...
	for (i = 0;  ; i++)
	{
		if (i >= MAX_LINE_LEN)
		{
			sysprintf("Line length >= MAX_LINE_LEN!!\n");
			while (1);
		}
...
int  get_next_pattern(void)
{
	...
	while (get_line() == 0)
	{
		...
		if (strncmp(p ,"Len", 3) == 0)
		{
			p += 3;
			p = find_alphanumeric(p);
			g_msg_len = str_to_decimal(p);
			sysprintf("Len = %d bits\n", g_msg_len);
			if (g_msg_len % 8)
				return -3;
			g_msg_len /= 8;
			continue;
		}
		if (strncmp(p ,"Outputlen", 9) == 0)
		{
			p += 9;
			p = find_alphanumeric(p);
			g_output_len = str_to_decimal(p);
			sysprintf("Outputlen = %d bits\n", g_output_len);
			if (g_output_len % 8)
				return -3;
			continue;
		}
		if (strncmp(p ,"Msg", 3) == 0)
		{
			p += 3;
			p = find_alphanumeric(p);
			len = str_to_hex(p, &g_sha3_msg[0], 0);
			sysprintf("message hex len = %d\n", len);
			if (len >= MAX_SHA_MSG_LEN)
			{
				sysprintf("SHA message length >= MAX_SHA_MSG_LEN!!\n");
				while (1);
			}
			continue;
		}
...
static int SHA3_test()
{
	...
	{
		case 224:
			g_sha3_mode = SHA_MODE_SHA224;
			break;
		case 256:
			g_sha3_mode = SHA_MODE_SHA256;
			break;
		case 384:
			g_sha3_mode = SHA_MODE_SHA384;
			break;
		case 512:
			g_sha3_mode = SHA_MODE_SHA512;
			break;
		default:
	...
	if (TSI_run_sha(1,                          /* inswap        */
					1,                          /* outswap       */
					SHA_MODE_SEL_SHA3,          /* mode_sel      */
					0,                          /* hmac          */
					g_sha3_mode,                /* mode          */
					0,                          /* keylen (hmac) */
					0,                          /* ks            */
					0,                          /* ks_num        */
					g_digest_len / 32,          /* wcnt          */
					g_msg_len,                  /* data_cnt      */
					ptr_to_u32(g_sha3_msg),     /* src_addr      */
	...
	do_swap((uint8_t *)&g_sha_digest[0], g_digest_len/8);

	if (do_compare(answer_digest, (uint32_t *)g_sha_digest, g_digest_len/32) < 0)
	{
		while (1);
	}
	do_swap((uint8_t *)&g_sha_digest[0], g_digest_len/8);
	...
	/*--------------------------------------------*/
	sysprintf("SHA3 without SWAP...\n");

	do_swap(g_sha3_msg, g_msg_len);
...
int  SHA3_KAT()
{
	...
	sysprintf("\n\n+---------------------------------------+\n");
	sysprintf("|  MA35D1 SHA3 Known-Answer-Test        |\n");
	sysprintf("+---------------------------------------+\n");

	g_sha3_msg = nc_ptr(sha3_msg_mem);
	...
	while (1)
	{
		// memset(g_sha3_msg, 0, sizeof(g_sha3_msg));
		ret = get_next_pattern();
		if (ret == -3)
		{
			sysprintf("Not byte aligned length, skipped.\n");
			continue;
		}
		if (ret < 0)
		{
