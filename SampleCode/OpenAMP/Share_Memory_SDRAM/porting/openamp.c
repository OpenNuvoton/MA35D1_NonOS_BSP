...
int OPENAMP_send_data(struct rpmsg_endpoint *ept, const void *data, int len)
{
    ...
    if (len == 0)
    {
        if (u8RxStart != TRUE)
        {
            /* Inform M4 that A35 TX test start */
            au32TxBuf[0] = COMMAND_SEND_TEST_START_TO_M4;
            au32TxBuf[1] = COMMAND_SEND_TEST_TX_TO_M4;
        }
        else
        {
            /* Inform M4 that A35 RX test start */
            au32TxBuf[0] = COMMAND_SEND_TEST_START_TO_M4;
            au32TxBuf[1] = COMMAND_SEND_TEST_RX_TO_M4;
        }
    }
    else
    {
        /* Inform M4 that A35 send data */
        au32TxBuf[0] = COMMAND_SEND_MSG_TO_M4;
        au32TxBuf[1] = u32TX_Length & 0xFF;
        au32TxBuf[2] = (u32TX_Length >> 8) & 0xFF;
        au32TxBuf[3] = (u32TX_Length >> 16) & 0xFF;

        memcpy((void *)(SHM_TX_START_ADDRESS | NON_CACHE), data, u32TX_Length);
    }
