...
struct rpmsg_device *platform_create_rpmsg_vdev(
    void *platform, unsigned int vdev_index, unsigned int role,
    void (*rst_cb)(struct virtio_device *vdev), rpmsg_ns_bind_cb ns_bind_cb)
{
    ...
    if (!shbuf_io)
        goto err1;
    shbuf =
        metal_io_phys_to_virt(shbuf_io, SHARED_MEM_PA /*+ SHARED_BUF_OFFSET*/);
    ...
    ret = ma35_rpmsg_init_vdev(rpmsg_vdev, vdev, ns_bind_cb, shbuf_io, &shpool);
    if (ret) {
...
static struct remoteproc *platform_create_rproc(int proc_index, int rsc_index)
{
    void *rsc_table;
    int rsc_size;
    int ret;
    ...
    (void *)remoteproc_mmap(&rproc_prime, &pa, NULL, rsc_size,
                            NORM_NSHARED_NCACHE | PRIV_RW_USER_RW,
                            &rproc_prime.rsc_io);

    /* mmap shared memory */
    pa = SHARED_MEM_PA;
    (void *)remoteproc_mmap(&rproc_prime, &pa, NULL, SHARED_MEM_SIZE,
