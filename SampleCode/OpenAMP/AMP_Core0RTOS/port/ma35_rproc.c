 ...
 */
static struct remoteproc *ma35_rproc_init(struct remoteproc *rproc,
                                          ...
                                          void *arg)
{
    ...
    ret = metal_device_open(prproc->kick_dev_bus_name, prproc->kick_dev_name,
                            &kick_dev);
    if (ret) {
...
static void *ma35_rproc_mmap(struct remoteproc *rproc, metal_phys_addr_t *pa,
...
static int ma35_rpmsg_receive_status(struct rpmsg_endpoint_priv *ept_priv,
                                     uint8_t sts)
{
    struct rsc_table_desc *desc;
    ...
    if (ret == pdTRUE) {
        if (rxqueue.cmd & VRING_DESC_CMD_HEAD) {
            ...
            if (uxQueueSpacesAvailable(ept_priv->xQueue))
                ma35_rpmsg_receive_status(
                    ept_priv, VRING_DESC_STS_ACK); // Its fine if no binding
...
static int ma35_rpmsg_desc_reset(struct rpmsg_endpoint *ept)
{
    ...
    while (desc) {
        ...
        desc = (desc->nxt_offset == 0)
                   ? NULL
                   : (struct rsc_table_desc *)(rproc_priv.shmem_base +
                                               desc->nxt_offset);
    }
...
static int ma35_rpmsg_send_offchannel_raw(struct rpmsg_endpoint *ept,
                                          ...
                                          const void *data, int len, bool wait)
{
    ...
    for (i = 0, res = len; i < len;
         i += rproc_priv.desc_txbuf, res -= rproc_priv.desc_txbuf) {
        if (res <= rproc_priv.desc_txbuf) {
            memcpy((void *)(rproc_priv.shmem_tx_base + desc->buf_offset),
                   (void *)((uint8_t *)data + i), res);
            desc->len = res;
        } else {
            memcpy((void *)(rproc_priv.shmem_tx_base + desc->buf_offset),
                   (void *)((uint8_t *)data + i), rproc_priv.desc_txbuf);
...
static int ma35_allocate_buf_pool(struct rpmsg_endpoint_priv *ept_priv,
                                  int size)
{
    int i, j;
...
/* used to bind desc remote to local. no need to scan if binded. return rx size
 * provide by remote  */
static int ma35_rpmsg_ns_bind_remote(struct rpmsg_endpoint *ept, char *ns,
                                     bool wait)
{
...
static int ma35_rpmsg_receive(struct rpmsg_endpoint_priv *ept_priv)
{
    ...
    while (desc && desc->len) {
        memcpy((void *)((char *)buf + rxlen),
               (void *)(rproc_priv.shmem_rx_base + desc->buf_offset),
               desc->len);
        rxlen += desc->len;
        desc = (desc->nxt_offset == 0)
                   ? NULL
...
static int ma35_request_tx_desc(struct rpmsg_endpoint *ept,
                                struct rpmsg_virtio_device *rvdev, int len)
{
    int i, ret;
    struct remote_resource_table *rsc_table = resource_table_shmem;
    struct rpmsg_endpoint_priv *ept_priv;
...
static int ma35_rpmsg_release_shmem(struct rpmsg_endpoint *ept)
{
    ...
    while (desc) {
        next =
            (struct rsc_table_desc *)(rproc_priv.shmem_base + desc->nxt_offset);
...
static int ma35_rpmsg_request_shmem(struct rpmsg_endpoint *ept,
                                    struct rpmsg_device *rdev, uint32_t len,
                                    bool wait)
{
    // request desc
...
static int ma35_rpmsg_create_ept(struct rpmsg_endpoint *ept,
...
int ma35_rpmsg_create_txept(struct rpmsg_endpoint *ept,
                            struct rpmsg_device *rdev, const char *name,
                            int size)
{
    int ret;
...
int ma35_rpmsg_create_rxept(struct rpmsg_endpoint *ept,
                            struct rpmsg_device *rdev, const char *name,
                            rpmsg_ept_cb cb)
{
    struct rpmsg_endpoint_priv *ept_priv;
    ...
    // create rxbuf queue
    ept_priv->xQueue =
        xQueueCreate(RXBUF_QUEUE_SIZE, sizeof(struct rxbuf_queue_t));
    if (!ept_priv->xQueue)
        goto err;
                         ...
                         struct rpmsg_virtio_shm_pool *shpool)
{
    ...
    rproc_priv.kick_ept =
        (void **)metal_allocate_memory(sizeof(void *) * rproc_priv.desc_num);
